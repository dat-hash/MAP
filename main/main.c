...
bool check_sum(const char *gnss_sentence) {
    ...
    // FIX: Check for '\0' end of string safety
...
void configure_gnss_module(void) {
    uint8_t frame[UBX_TX_BUFFER_SIZE];
    size_t index = ubx_build_valset_header(frame, UBX_LAYER_RAM | UBX_LAYER_BBR | UBX_LAYER_FLASH);
...
void app_main(void) {
    ...
    // Ensure UBX frame completely clears host hardware before changing baud rate
